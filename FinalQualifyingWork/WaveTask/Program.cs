    ...
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                ConsoleKey key;
                WaveLogic wave = new WaveLogic();
                do
                {
                    Console.ForegroundColor = ConsoleColor.White;
                    Console.WriteLine("Справка:");
                    Console.WriteLine("\tl - выключить логирование");
                    ...
                    logging = s.IndexOf("l") != -1 ? false : true;
                    ...
                    if (multiInputComma || multiInputDefis)
                    {
                        if (multiInputComma)
                        {
                            ...
                            for (int i = 0; i < arrN.Length; i++)
                            {
                                try
                                {
                                    ...
                                    {
                                        wave.setStartParameters(int.Parse(arrN[i].Split(' ')[0]), logging);
                                    }
                                    else
                                    {
                                        wave.setStartParameters(int.Parse(arrN[i]), logging);
                                    }
                                }
                                catch (Exception e)
                                {
                        ...
                        else
                        {
                            ...
                            for (int i = begin; i <= end; i++)
                            {
                                try
                                {
                                    wave.setStartParameters(i, logging);
                                }
                                catch (Exception e)
                                {
                                    viewError(e);
                    ...
                    else
                    {
                        ...
                        wave.setStartParameters(N, logging);
                    }
                    Console.ForegroundColor = ConsoleColor.Red;
                    //Console.WriteLine(String.Format("Jstar = {0:F50}", wave.Jstar));
                    Console.ForegroundColor = ConsoleColor.White;
                    ...
                    if (key == ConsoleKey.S)
                    {
                        ...
                        while (error)
                        {
                            try
                            {
                                ...
                                foreach (string str in wave.screenLog)
                                {
                                    writer.WriteLine(str);
                                }
                                writer.Close();
                                error = false;
                                wave.screenLog.Clear();
                            }
                            catch (Exception e)
                            {
                                viewError(e);
